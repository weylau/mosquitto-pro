...
int main(int argc, char *argv[])
{
	...
	log__printf(NULL, MOSQ_LOG_INFO, "Connecting to redis host %s on port %d.", config.redis_host, config.redis_port);
	int_db.redis_context = redisConnectWithTimeout(config.redis_host, config.redis_port, timeout);
	if (int_db.redis_context == NULL || int_db.redis_context->err) {
		if (int_db.redis_context) {
			log__printf(NULL, MOSQ_LOG_ERR, "Error: Redis connection error %s.", int_db.redis_context->errstr);
		} else {
			log__printf(NULL, MOSQ_LOG_ERR, "Error: Redis can't allocate redis context.");
...
#ifdef WITH_REDIS
	redisReply *reply;
	reply = redisCommand(int_db.redis_context, "DEL %s", int_db.config->redis_key);
