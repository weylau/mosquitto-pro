...
#ifdef WITH_REDIS
					if(reload) continue;
					if(conf__parse_string(&token, "redis_host", &config->redis_host, saveptr)) return MOSQ_ERR_INVAL;
#endif
				}else if(!strcmp(token, "redis_port")){
#ifdef WITH_REDIS
					if(reload) continue;
					if(conf__parse_int(&token, "redis_port", &config->redis_port, saveptr)) return MOSQ_ERR_INVAL;
#endif
				}else if(!strcmp(token, "redis_key")){
#ifdef WITH_REDIS
					if(reload) continue;
					if(conf__parse_string(&token, "redis_key", &config->redis_key, saveptr)) return MOSQ_ERR_INVAL;
#endif
				}else{
